...
function Counter({ value, suffix = "" }: { value: number; suffix?: string }) {
  ...
  const fmt = (v: number) =>
    value >= 1000
      ...
      : v.toFixed(value < 10 ? 1 : 0).replace(".", ",");
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="fixed bottom-4 left-3 right-3 z-[100] sm:hidden"
      >
        <div className="flex items-center gap-2 p-3 rounded-2xl border border-fuchsia-500/30 shadow-[0_8px_30px_rgba(168,85,247,0.4)]"
          style={{ background: "linear-gradient(135deg,#1a0533,#0d0d1f)" }}>
          <Link href="/app" className="flex-1 py-3 rounded-xl bg-gradient-to-r from-fuchsia-500 to-violet-500 text-white font-bold text-sm text-center shadow-[0_0_20px_rgba(168,85,247,0.4)]">
            🚀 Teste grátis — sem cartão
          </Link>
          <button onClick={() => setDismissed(true)} className="w-9 h-9 flex items-center justify-center rounded-xl bg-white/5 text-zinc-500 hover:text-white flex-shrink-0 text-base">
            ✕
          </button>
        </div>
      </motion.div>
    ...
    <div className="min-h-screen text-white overflow-x-hidden">
      ...
      <section className="relative z-10 px-6 pt-24 pb-16 max-w-6xl mx-auto text-center">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45, duration: 0.6 }}
          className="max-w-xl mx-auto mb-5 flex flex-col sm:flex-row items-center justify-center gap-3">
          <Link href="/app"
            className="group w-full sm:w-auto flex items-center justify-center gap-3 px-10 py-5 rounded-2xl bg-gradient-to-r from-fuchsia-500 to-violet-500 text-white font-bold text-lg shadow-[0_0_50px_rgba(217,70,239,0.5)] hover:shadow-[0_0_70px_rgba(217,70,239,0.7)] hover:scale-[1.03] transition-all whitespace-nowrap">
          ...
          <a
            ...
            className="w-full sm:w-auto flex items-center justify-center gap-3 px-6 py-5 rounded-2xl border border-white/15 bg-white/5 hover:bg-white/10 hover:border-white/25 transition-all"
          >
            ...
            <div className="text-left">
              <div className="text-[10px] text-zinc-400 leading-none mb-0.5">Baixe o app</div>
      ...
      <motion.section id="como-funciona" className="relative z-10 px-6 py-32 max-w-6xl mx-auto" initial="hidden" whileInView="show" viewport={REVEAL} variants={stagger}>
        ...
        <motion.div variants={fadeUp} className="mt-16 text-center">
          <Link href="/app"
            className="inline-flex items-center gap-2 px-8 py-4 rounded-xl border-2 border-fuchsia-500/50 text-fuchsia-300 font-bold text-base hover:bg-fuchsia-500/10 hover:border-fuchsia-400 transition-all">
            <Sparkles className="w-4 h-4" />
            Testar agora — 10 cortes grátis, sem cartão →
          </Link>
          <p className="text-xs text-zinc-600 mt-3">Sem instalar nada · pronto em menos de 3 minutos</p>
        </motion.div>
      </motion.section>
      ...
      <motion.section id="planos" className="relative z-10 px-6 py-24 max-w-7xl mx-auto" initial="hidden" whileInView="show" viewport={REVEAL} variants={stagger}>
        ...
        {/* Banner urgência */}
        <motion.div variants={fadeUp} className="max-w-lg mx-auto mb-10">
          <div className="flex items-center justify-center gap-2.5 px-4 py-3 rounded-xl border border-amber-400/20 bg-amber-500/5 text-sm text-center">
            <span className="relative flex h-2 w-2 flex-shrink-0">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-amber-400 opacity-75" />
              <span className="relative inline-flex rounded-full h-2 w-2 bg-amber-400" />
            </span>
            <span className="text-amber-200/80"><strong className="text-amber-300">47 pessoas</strong> visitando os planos agora · 7 dias de garantia em todos os planos</span>
          </div>
        </motion.div>
              ...
              <motion.div key={plan.name} variants={fadeUp} className="flex flex-col relative">
                ...
                <GlowCard customSize glowColor={plan.glow} className="flex flex-col flex-1 relative overflow-hidden">
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    {/* Garantia */}
                    <p className="text-[11px] text-emerald-400/70 text-center mb-4 flex items-center justify-center gap-1">
                      <Check className="w-3 h-3 text-emerald-500" /> 7 dias de garantia · cancele com 1 clique
                    </p>
