...
const REVEAL_VIEW = { once: true, amount: 0.2 } as const;
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#050507] text-white overflow-x-hidden">
      ...
      <motion.section
        id="como-funciona"
        className="relative z-10 px-6 md:px-10 py-24 max-w-6xl mx-auto"
        initial="hidden"
        whileInView="show"
        viewport={REVEAL_VIEW}
      ...
      >
        ...
        <motion.div className="grid md:grid-cols-3 gap-6 mb-16" variants={staggerParent}>
          {STEPS.map((step, i) => (
            <motion.div
              key={step.num}
              variants={fadeUp}
              whileHover={{ y: -6, transition: { duration: 0.25, ease: "easeOut" } }}
              className="card-border-hover rounded-3xl p-8 relative"
            >
              {i < STEPS.length - 1 && (
      ...
      <motion.section
        id="planos"
        className="relative z-10 px-6 md:px-10 py-24 max-w-7xl mx-auto"
        initial="hidden"
        whileInView="show"
        viewport={REVEAL_VIEW}
        variants={staggerParent}
        ...
        >
          {PLANS.map((plan) => (
            ...
            >
              {plan.badge && (
                <motion.div
                  className="absolute -top-4 left-1/2 -translate-x-1/2 btn-primary text-xs font-black px-4 py-1.5 rounded-full whitespace-nowrap tracking-wider"
                  animate={{ y: [0, -3, 0] }}
